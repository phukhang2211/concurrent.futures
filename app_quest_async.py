...
def firestore_to_csv_paginated_sub_collections(db, ctr, fields_to_read,col_name,scn, csv_filename='extract.csv', max_docs_to_read=-1, write_headers=True):
    """ Extract Firestore collection data and save in CSV file
    Args:
        db: Firestore database object
...
#for col_name in executor.submit(collection_names):
def _run(col_name):
        ...
        for user_id in user_id_array:
            ...
            for scn in sub_col_name:
                ...
                if len(flds) == 0:
                    ...
                    # Extract docs from Firestore
                    firestore_to_csv_paginated_sub_collections(db, ctr, csv_headers, csv_filename,col_name, scn)
...
start = time.time()
for col_name in executor.map(_run,collection_names):
    print("==========")
...
start = time.time()
for folder in executor.summit(folders):
